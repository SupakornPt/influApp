      ...
      <Modal open={active === "draft"} title="Draft management" onClose={close}>
        ...
        {variant === "influencer" ? (
          <div className="mt-3 flex gap-2">
            ...
            />
            <button type="button" onClick={addDraft} className={cn("shrink-0 rounded-lg px-3 py-1.5 text-xs font-semibold transition", pageBtn)}>
              Add draft
            </button>
          </div>
        ) : null}
      ...
      <Modal open={active === "work"} title="Published content" onClose={close}>
        {variant === "influencer" ? (
          <>
            ...
            />
            <button type="button" className={cn("mt-3 rounded-lg px-3 py-2 text-xs font-semibold transition", pageBtn)} onClick={close}>
              Save link
            </button>
          </>
        ) : (
